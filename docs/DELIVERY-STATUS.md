...
5. **Finish history reader integration (D9).** Add legal-date and knowledge-cutoff queries only after D8, with partial/unavailable results and source basis. Connect paragraph histories to exact operation evidence. Stable citation links work; Public subscription clipboard delivery is now verified; see the latest execution position below.
...
## Latest execution position — 26 September 2026

- History intake [36233058951](https://github.com/sondreskarsten/norwegian-laws-history/actions/runs/36233058951) published v7 product `044f1fc7ac178149dc0a0a022e48dfd26ca579e5db845b56c271c4ac436ec8f5`: 5,247 qualified and 627 rejected bodies. Its publication commit is `9f93e0fbb15d0202d31564b3aefcb4404ff075e5`. Reader deployment and independent v7 regeneration remain pending; reader v6 acceptance remains valid.
- v8 literal-list qualification supports 5,351 bodies locally, including 104 additions verified against retained raw XML. All 5,247 v7 HTML/CSS payloads remain byte-identical. There are 523 remaining structural rejections. The sampled compound labels were inspected in a 390px document frame. See `docs/evidence/source-body-list-labels-v8-local.json`.
- The older-source acquisition bundle is published and independently downloaded by a fresh Windows installation. It preserves source evidence, not qualified historical intervals. See `docs/evidence/primary-source-public-readback.json`.
- Public clipboard delivery is verified. Constrained-network title/full-text search has recorded delivery evidence in the reader repository. The watcher recovery repair is reviewed; live issue creation remains unexercised. A literal fork requires a different eligible account or approved organization: the authenticated owner cannot fork its own repository.
- Ordered amendment trees are implemented on the existing branch, with raw source locations and unresolved associations retained. Complete operation classification, eligible legal timing, reconstruction and legal-date reader navigation remain unfinished. GCS configuration audit is prepared, not yet run. No remaining issue is closed by these partial milestones.

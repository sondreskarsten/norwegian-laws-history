...
Work paused at the user's request on **26 September 2026**. This is the restart point; older dated reports remain evidence of their own runs. Both repositories remain in `github.com/sondreskarsten`. Reuse `fix/reliable-laws-delivery` and `fix/reliable-history-delivery`; preserve generated branches, historical tags and immutable evidence. No cloud handoff or DNB migration is required.
...
- Public observed-history reader: version selection, changed/unchanged comparison, unsupported/unknown selection handling, pinned source downloads and mobile layouts are verified. Last complete reader receipt/acknowledgement readback is source `741ca3ccfab2663756019e58795f8263594baa27` (main PR21).
...
|---|---|---|
| [Main 36222112299](https://github.com/sondreskarsten/norwegian-laws/actions/runs/36222112299) | Deploy main PR22 metadata/typography support and refresh observed-history inputs | In progress; formatting/Quarto generation. Public v6 reader readback still required. |
...
| [History 36222784579](https://github.com/sondreskarsten/norwegian-laws-history/actions/runs/36222784579) | Independently regenerate exact v6 product with networking denied during generation | In progress; fetch final `body-reproduction.json` and assess exact identity. |
...
The previously published reader remains the verified baseline while its refresh runs. No legal-date functionality is claimed by the running jobs. Existing daily workflows remain enabled; no new automation was created.

## Remaining delivery, in order

1. **Finish the in-flight release acceptance.** Read the final main publication receipt, compare the committed acknowledgement, inspect the new public history index and representative v6 bodies, and verify the exact v6 reproduction report. Record any failure before retrying. Public release bytes are verified; that does not by itself verify the refreshed website.
...
6. **Finish continuing operation and portability (D2/D5/D10).** Verify a subsequent scheduled daily cycle, literal clean-fork operation, controlled slow-network search, notification deduplication and actual supported integrations. External messages require a user-designated destination. Audit optional GCS retention/IAM; current-object checks do not establish retention/access correctness.

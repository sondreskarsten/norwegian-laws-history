...
| 3. Structures | Published v7 product `044f1fc7ac178149dc0a0a022e48dfd26ca579e5db845b56c271c4ac436ec8f5` has 5,247 qualified bodies and 627 explicit rejections. v8 adds 104 raw-qualified bodies, v9 adds 24 and v10 adds 68, reaching 5,443 locally. Each batch preserves all earlier qualified HTML/CSS payloads. | Combined v8/v9/v10 release; 431 remaining v10 structural rejections including images, formulas, tables and other source forms. Implementable structures remain defects/work, not completed limitations. |
...
Image acquisition remains incomplete: the retained source archives contain no image binaries. Public range probes returned 300 valid image prefixes, followed by 643 HTTP 403 responses consistent with a request cap. The burst stopped; these failures do not establish missing assets. A later single full-file request returned HTTP 200 with a verified PNG signature, and serial full acquisition is now running with a delay and stop-on-error policy. Formulas are retained as TeX text and require a pinned offline renderer. See `docs/evidence/remaining-source-structures/README.md`.
...
Current v7 publication/readback is complete. Merge the reviewed primary-source, ordered-evidence, watcher and combined v8/v9/v10 batch; publish and independently read the resulting product once. Continue grouped table/assets/formula support and source qualification, followed by eligible reconstruction and the legal-date reader. Complete operational acceptance and map each issue to published evidence before final closure.
